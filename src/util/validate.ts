...
const V = {
    ...
    many: (rules: Validator[]) => (d: any): string | false => rules.reduce((err, rule) => err ? err : rule(d), false),
    ...
    hasKey: (key: string) => V.many([
        V.object,
        (d: any) => (key in d) ? false : `Expected object to have key '${key}'`
    ]),

    arrayWithEach: (validate: Validator) => V.many([
        V.array,
        (d: any[]): string | false => {
